...
Yours truly could be wrong, but one advantage of `geojson` and `json` is that it minimizes on size (sometimes) and is more portable than shapefiles. Shapefiles are dependent on other data formats that accompany it, such as `.shx`, `.dbf`, `.prj` and others that provide geospatial orientation, metadata, attributes and others. GeoJSON and JSON formats will come as standalone and holding the same amount of data. 
...
```{r}
knitr::include_graphics(rep("D:/gachuhi/my-leaflet/nairobi-named.jpg"))
```

Now create a new pin over *Kisumu* and a new dictionary will appear below that of Nairobi and a new table row will appear in the **Table** tab. Do the rest for the following cities: Mombasa, Nakuru, Nyeri, Machakos and Malindi. Constitutionally speaking, only the first four are cities legally, the rest are just towns but for the sake of this tutorial, let's corporately refer to them as cities.

```{r}
knitr::include_graphics(rep("D:/gachuhi/my-leaflet/cities-named.jpg"))
```
...
```{r}
knitr::include_graphics(rep("D:/gachuhi/my-leaflet/geojson-save.jpg"))
```

## Saving the Geojson to Github

Now, based on experience, using a local Geojson file (one within your computer directory) is painful in JavaScript. The error had to do with servers or something. To get a way around this and still be able to display `.geojson` data in Leaflet, the data had to be stored on an online server, in this case Github. We would love to show you how to save data on Github, but this would make this chapter long and we would like you Google it out. Apologies.

However, on the bright side, we have provided the same GeoJson we saved before which you can access from this [link](https://raw.githubusercontent.com/sammigachuhi/geojson_files/main/cities-geojson.geojson). 
...
First of all create blank JavaScript file called `geojson.js`. Thereafter, go to your `map.html` file which you had created last in [Chapter 2][Setting the superstructure] Open it. It should like below.
...
Inside the `<script>` tag of `<div>` id *myMap* change the `src` to reference to your `geojson.js` file. Alright. Head over to your `geojson.js` and as always, add the leaflet classes `L.map` and `L.tileLayer`. We set the view of our new Leaflet map to that of Nairobi. Your blank `geojson.js` should now be filled with the below code.
...
```{r}
knitr::include_graphics(rep("D:/gachuhi/my-leaflet/geojson-copy.jpg"))
```
...
```{r}
knitr::include_graphics(rep("D:/gachuhi/my-leaflet/geojson-names.jpg"))
```


There is one issue with this method. If we have a very long GeoJSON data structure, it will clatter our JavaScript file. We only worked with seven cities, but it is very common to work with data holding hundreds and even thousands of dictionaries. 
...
This file will allow to add you to add `.geojson` files to your Leaflet map. However, there is a catch, both the Ajax and Fetch APIs only work with Geojson file formats saved on a web server. Based on experience, they will not work with local geojson files. As a work around, we saved our GeoJSON file to github. Don't worry, here is the [link](https://raw.githubusercontent.com/sammigachuhi/geojson_files/main/cities-geojson.geojson) to the raw `.geojson` file we had created earlier.

We shall call our GeoJSON file from Github using Ajax using the code below. Please remember to comment out your `var cities` and `L.geoJSON` using `\\`.
...
Your map should now show the markers of our cities. For those hawkeyed people, you may have noticed that the syntax is a bit different, both from the Ajax Website and that of the `L.geoJSON` from Leaflet. Starting with the latter, we have instead used `L.geoJson` and unlike in the creators website where he used `var geojsonLayer = new L.GeoJSON.AJAX(<your-geojson-file>)` we used the following syntax: `L.geoJson.ajax()`. Actually, that's what worked after a lengthy web search.
...
Fetch API is  a modern interface that allows you to make HTTP requests to servers from web browsers. In other words, it searches for a resource over the web and retrieves it, thus giving it back to you. 
...
After converting our response to JSON, it still needs to be processed further. Processed to what? To GeoJSON file and subsequently add it to our Leaflet Map.
...
To see the `catch()` in action, omit the last letter in our url so that it reads `cities-geojson.geojso`. Reload your `map.html`. Right the webmap page and click on **Inspect**. Head over to the console tab and see the error response. It should read like this:

```{r}
knitr::include_graphics(rep("D:/gachuhi/my-leaflet/catch-error.jpg"))
```

Restore the omitted letter and reload your `map.html`. Your leaflet map should have the city markers overlain just like in the case of using Ajax plugin or hardcoding the Geojson into the `var cities`. To stretch your Javascript skills further, we can shorten our code further by retaining the arguments `response`, `data` and `error` and using the arrow function `=>` to pass on the return statements, like so.
...
It looks cryptic but don't cry! We have only ommitted the `function()` keyword and instead added `=>` between `function()` and the {} which is the function body aka where the magic happens. Just like when using Ajax, we can also add other functionalities within `L.geoJSON`. In here, and thanks to the use of template literals `(``)`, we can even add statements and refer our variables using `${}`. Whatever is within the `${}` is executed and passed out as a string to the template literals.
...
```
In our case, we added the HTML tag `<br>` to separate the City and Population variables. What we have are neat markers showing both the city name and population figures in two separate lines. 

Enough Javascript for a day!


```{r}
knitr::include_graphics(rep("D:/gachuhi/my-leaflet/city-population.jpg"))
```
